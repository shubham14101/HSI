...
with tf.Session(config=config) as sess:
	...
	#	print("no model")
	for k in range(47):
		training_data_index = np.random.permutation(training_data_index)
		print(training_data_index.shape)
		lossT = 0
		acc = 0
		acc2= 0
		for j in range(1800):
			...
			if(label[training_data_index[(j),0],training_data_index[(j),1]]==14):
				tempECD[8] = 1
			#print(tempECD)
			inp = np.array([tempINP])
			encoding = np.array([tempECD])
			...
			#print(ansP[0,:].shape)
			for q in range(4):
				if(np.argmax(ansP[q,:])==np.argmax(encoding[q,:])):
					acc = acc + 1
				if(np.argmax(final[q,:])==np.argmax(encoding[q,:])):
					acc2 = acc2 + 1
		print(lossT)
		print(acc/7200,accP/7434)
		print(acc2/7200,accP2/7434)
		if(k%5 == 0):
			save_path = saver.save(sess, "/tmp/model2.ckpt")
			save_path2 = saver.save(sess,"D:/HSI/model2/model2.ckpt")
			accP = 0
			accP2 = 0
			...
			for l in range(7434):
				...
				if(np.argmax(finalPred)==np.argmax(encoding)):
					...
					np.savetxt('IndianPinesClass2.txt',classwise)